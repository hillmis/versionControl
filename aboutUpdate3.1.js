...
function toAbout() {
    ...
    versionBtn.className = 'about-btn version-btn';
...
`;
    ...
    authorBtn.href = 'https://home.liu13.fun';
    authorBtn.className = 'about-btn author-btn';
...
`;
    ...
    btnContainer.append(versionBtn, authorBtn);
    //tips
    ...
    thanks.textContent = '❤️感谢您的支持，我们将持续改进！';
    ...
    // 在toAbout()函数中修改这部分：
    const poemText = document.querySelector('meta[name="description"]')?.content || '诗云...';
    textContainer.append(
        document.createTextNode(poemText),
        btnContainer,
        tips,
        donateSection
    );
    ...
    // 组装元素
    content.append(closeBtn, logo, title, textContainer);
...
function checkAndAddMetaDescription() {
    ...
    fetchPoem().then(text => {
        meta.content = text;
        // 同时更新关于弹窗中的诗句文本
        const poemElement = document.querySelector('.text-container');
        if (poemElement) {
            poemElement.firstChild.textContent = text;
        }
    });
}
// 配套CSS样式
...
.version-btn {
    background: rgba(41, 201, 97, 0.12);
    color: #29c961;
    border: 1px solid rgba(41, 201, 97, 0.24);
}
...
// 配置区（正式使用需替换以下参数）
const CONFIG = {
    OFFICIAL_SIGNATURE: 'C2E95E8A32CFCB657F1AFE76269214BB', // 官方正确MD5
